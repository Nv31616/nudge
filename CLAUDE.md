...
- `data/export/RuleExportData.kt` — data classes: `NudgeExport`, `ExportedRule`, `ExportedGroup`, `ExportedHistoryEvent`
...
- `domain/usecase/ExportRulesUseCase.kt` — collects enabled rules + groups + members + the full usage history, delegates to RuleExporter (off the main thread)
- `domain/usecase/ImportRulesUseCase.kt` — `preview()` (suspend) returns an `ImportPreview`, `execute()` inserts with duplicate detection (packageName + mode + schedule match). Creates groups by name if missing. Merges history last.
...
- Export format: pretty-printed JSON, version 1, human-readable. Groups referenced by name (not ID). Usage history rides in the same file — see below.
...
- **No schema migration.** Dedup happens at import time: `HistoryMerge.timestampRange` bounds a `UsageEventDao.getKeysInRange` projection (4 columns, only the window the file covers) into a set, `selectNew` filters, `UsageRepository.insertEvents` batch-inserts in chunks of 500. `ImportRulesUseCase.newHistoryEvents` is shared by `preview` and `execute`, and execute RE-READS rather than trusting the preview's number.
